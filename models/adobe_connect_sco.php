	...
	*/
	public function save($data = null, $validate = false, $fieldList = array()) {
		...
		if (isset($data[$this->primaryKey]) && !empty($data[$this->primaryKey])) {
			...
			$this->errors[] = "Error: ".__function__." Sorry, you must have either a folder-id or a sco-id to save a SCO";
			return false;
		...
		if (isset($this->response['Status']['Invalid'])) {
			$this->errors[] = "Invalid Fields: ".json_encode($this->response['Status']['Invalid']);
			return false;
		}
